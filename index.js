...
pizza_app.post('/order_page', (req, res) => {
    ...
    if (firstName == "" || lastName == "" || sorts == undefined || width == 0 || size == 0 || phone == "" || address == "" || !regexPhone.test(phone)) {
        ...
        res.render('order_page', { quantity: quantity, sorts: sorts, size: size, width: width, firstName: firstName, lastName: lastName, phone: phone, address: address, pizzaPrice: pricePizza, sizePrice: sizePrice, widthPrice: widthPrice, sum_sub: sum_sub, taxesValue: taxesValue, totalValue: totalValue });
    }
});
